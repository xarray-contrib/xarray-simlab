...
def test_to_graphviz_inout_edges():
    #
    #in1->io1->in2->io2
    # \<- - - - - - -/
    @xs.process
    ...
    model = xs.Model({"in1":In1,"in2":In2,"io1":Inout1,"io2":Inout2}, custom_dependencies={"io2":"in2","in2":"io1","io1":"in1"})
    g = to_graphviz(model, show_feedbacks=True)
    actual_nodes = _get_graph_nodes(g)
    actual_edges = _get_graph_edges(g)
    expected_nodes = list(model)
    expected_edges = [
