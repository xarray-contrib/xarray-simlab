...
from xsimlab.diagnostics.base import runtime_hook, RuntimeDiagnostics
...
def test_runtime_hook_call_frozen(model):
    ...
    with pytest.raises(TypeError, match=".*not support item assignment"):
        in_ds.xsimlab.run(model=model, diagnostics=[change_context])
    ...
    with pytest.raises(TypeError, match=".*not support item assignment"):
        in_ds.xsimlab.run(model=model, diagnostics=[change_state])


@pytest.mark.parametrize("given_as", ["argument", "context", "register"])
def test_runtime_diagnostics_instance(model, given_as):
    flag = [False]
...
def test_runtime_diagnostics_init():
    def not_a_decorated_hook(model, context, state):
        pass

    with pytest.raises(TypeError, match=".*only runtime_hook decorated.*"):
        RuntimeDiagnostics(not_a_decorated_hook)


def test_runtime_diagnostics_subclass(model):
    flag = [False]
